...
Modified by Daniel Ford and Josh Chang

Sets up the database with all of the information needed from our web pages.
'''
...
class WebDB(object):
   ...
   def lookupItemNameType(self, itemID):

       sql = "SELECT name, type FROM Item WHERE id=%d"\
           % (itemID)
       res = self.execute(sql)
       reslist = res.fetchall()
       if reslist == []:
   ...
   def superDoc(self, docID):
        sql = "SELECT itemID FROM UrlToItem WHERE id = %d" \
             % (docID)
        res = self.execute(sql)
        reslist = res.fetchall()
        if reslist == []:
